...
function buildCopyPayload(o: any) {
  ...
  const phone = normalizePhoneToPlus10(
    details.receiver_phone ?? o.customer_phone ?? details.phone ?? o.contact ?? o.customer_phone
  );
...
function getItemTitle(it: any): string {
  return it?.item_name || it?.product_title || it?.product_variant?.product?.title || "Item";
}
                ...
                return (
                  <div key={o.id} className="rounded-2xl border border-white/10 bg-bg-900/30 p-4">
                    ...
                    <div className="mt-2 text-sm text-white/70">
                      Status: <span className="text-white/90">{o.status}</span> • Payment: <span className="text-white/90">{o.payment_status}</span> • Shipping:{" "}
