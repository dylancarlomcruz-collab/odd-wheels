...
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "card relative rounded-2xl border border-white/10 bg-bg-900/60 shadow-soft backdrop-blur-sm",
...
export function CardHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn("card-header relative border-b border-white/10 bg-bg-950/30 p-5 backdrop-blur-sm", className)}
      {...props}
    />
  );
}
